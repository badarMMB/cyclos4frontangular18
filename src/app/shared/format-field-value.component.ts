...
export class FormatFieldValueComponent extends AbstractComponent implements OnInit {
  ...
  get showCountdownTimer(): boolean {
    const internalName = (this.field?.internalName || this.fieldName || '').toLowerCase();
    return this.type === CustomFieldTypeEnum.DATE && internalName.includes('countdown') && this.countdownTargetDate !== '';
  }

  get isMapUsersField(): boolean {
    return this.field?.internalName === 'mapUsers' && this.type === CustomFieldTypeEnum.RICH_TEXT;
  }

  get countdownTargetDate(): string {
    const raw = this.fieldValue?.dateValue ?? this.internalValue ?? this.value;
