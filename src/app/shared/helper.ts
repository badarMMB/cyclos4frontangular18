...
import { INgxGalleryImage } from 'app/shared/ngx-gallery-compat';
...
export function locateControl(
  ...
  if (result instanceof UntypedFormGroup) {
    if (locator.property) {
      ...
      if (customValues instanceof UntypedFormGroup) {
 ...
 */
export function validateBeforeSubmit(control: AbstractControl, returnNonValid = false): boolean | UntypedFormControl[] {
  ...
  if (control instanceof UntypedFormControl) {
    ...
    control.controls.forEach(current => {
      Array.prototype.push.apply(result, validateBeforeSubmit(current, true) as UntypedFormControl[]);
    });
  } else if (control instanceof UntypedFormGroup) {
...
export function cloneControl<C extends AbstractControl>(control: C): C {
  ...
  if (control instanceof UntypedFormGroup) {
    clone = new UntypedFormGroup({}, control.validator, control.asyncValidator);
    ...
    clone = new UntypedFormArray(clones, control.validator, control.asyncValidator);
  } else if (control instanceof UntypedFormControl) {
    clone = new UntypedFormControl(control.value, control.validator, control.asyncValidator);
  }
  return clone;
}
