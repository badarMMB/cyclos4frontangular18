...
import { ControlContainer, UntypedFormArray, UntypedFormControl, NG_VALUE_ACCESSOR } from '@angular/forms';
...
export class InputListFieldComponent extends BaseFormFieldComponent<string[]> implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.array = new UntypedFormArray([new UntypedFormControl(null)]);
    this.setupArraySubs();
  }
  ...
  add() {
    this.array.insert(this.array.length, new UntypedFormControl(null));
    this.setupArraySubs();
  ...
  writeValue(obj: any) {
    ...
    if (array.length === 0) {
      ...
      this.array = new UntypedFormArray(array.map(v => new UntypedFormControl(v)));
    }
    this.setupArraySubs();
