...
export class PaymentStepFormComponent extends BaseComponent implements OnInit {
  ...
  @Input() customValuesControlGetter: (cf: CustomFieldDetailed) => UntypedFormControl;
  ...
  private adjustPaymentTypes() {
    ...
    const currentFormValue: string = this.form.value.type;
    const matchedType = allPaymentTypes.find(
      t =>
        t.id === currentFormValue ||
        t.internalName === currentFormValue ||
        t.to?.internalName === currentFormValue ||
        t.to?.id === currentFormValue
    );

    // If we received a type / account internal name that belongs to another source account,
    // first switch account and let the account subscription re-run this method.
    if (matchedType?.from?.id && matchedType.from.id !== value.account) {
      this.form.patchValue({ account: matchedType.from.id });
      return;
    }

    // Filter the payment types from the selected account type
    const paymentTypes = allPaymentTypes.filter(tt => tt.from.id === this.form.value.account);
    ...
    if (empty(paymentTypes)) {
      ...
      if (matchedType && paymentTypes.find(t => t.id === matchedType.id)) {
        // Convert internal name (or destination account ref) to the technical transfer type id.
        type = matchedType.id;
      }
