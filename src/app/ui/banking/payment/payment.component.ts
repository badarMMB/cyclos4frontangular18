...
export class PaymentComponent extends BasePageComponent<DataForTransaction> implements OnInit {
  ...
  lastValue: any;
  customValuesControls = new Map<string, UntypedFormControl>();
  ...
  ngOnInit() {
    ...
    if (!this.pos) {
      ...
      this.typeParam = route.queryParamMap.get('type');
      this.toSelf =
        this.toParam != null &&
  ...
  onDataInitialized(data: DataForTransaction) {
    if (empty(data.accounts)) {
      ...
      if (!empty(this.typeParam)) {
        this.form.patchValue({ type: this.typeParam }, { emitEvent: true });
      }
  ...
  toConfirm() {
    ...
    this.form.setControl('customValues', new UntypedFormGroup(customValueControls));
