  ...
  implements OnInit
{
  ...
  protected setupForm(form: UntypedFormGroup, data: VoucherDataForRedeem): void {
    if (data.type.allowPartialRedeems) {
      form.addControl('amount', new UntypedFormControl(null, Validators.required));
    }
    if (data.pinInput) {
      form.addControl('pin', new UntypedFormControl(null, Validators.required));
    }
  }
  ...
  validatePerformFromForm(_data: VoucherDataForRedeem, form: UntypedFormGroup): boolean {
