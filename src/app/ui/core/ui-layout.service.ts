...
export class UiLayoutService {
  ...
  private readonly _leftAreaVisible = signal(false);
  readonly leftAreaVisibleSignal = this._leftAreaVisible.asReadonly();
  leftAreaVisible$ = new BehaviorSubject(false);
  get leftAreaVisible(): boolean {
    return this._leftAreaVisible();
  }
  set leftAreaVisible(visible: boolean) {
    if (visible !== this._leftAreaVisible()) {
      this._leftAreaVisible.set(visible);
