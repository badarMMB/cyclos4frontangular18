...
export class ImportFileComponent extends BasePageComponent<ImportedFileDataForNew> implements OnInit {
  ...
  confirmationPasswordControl: UntypedFormControl;
  ...
  onDataInitialized(data: ImportedFileDataForNew) {
    ...
    if (data.confirmationPasswordInput) {
      this.confirmationPasswordControl = new UntypedFormControl('', Validators.required);
    }

    const defaults = data.importedFile;
    switch (data.kind) {
      case ImportedFileKind.ADS:
        this.form.addControl('currency', new UntypedFormControl(defaults.currency, Validators.required));
        break;
      case ImportedFileKind.PAYMENTS:
        this.form.addControl('sendNotifications', new UntypedFormControl(!!defaults.sendNotifications));
        break;
      case ImportedFileKind.RECORDS:
        this.form.addControl('recordType', new UntypedFormControl(defaults.recordType, Validators.required));
        break;
      case ImportedFileKind.USERS:
        this.form.addControl('useGroupFromFile', new UntypedFormControl(defaults.useGroupFromFile));
        this.form.addControl('group', new UntypedFormControl(defaults.group, GROUP_VALIDATOR));
        this.form.addControl('sendActivationEmail', new UntypedFormControl(defaults.sendActivationEmail));
        break;
      case ImportedFileKind.USER_PAYMENTS:
        this.form.addControl('paymentType', new UntypedFormControl(defaults.paymentType, Validators.required));
        break;
      case ImportedFileKind.USER_SEND_VOUCHERS:
        this.useAmountFromFile$.next(!!defaults.useAmountFromFile);
        const useAmountFromFileControl = new UntypedFormControl(this.useAmountFromFile$.value);
        this.addSub(useAmountFromFileControl.valueChanges.subscribe(v => this.useAmountFromFile$.next(v)));
        this.form.addControl('voucherType', new UntypedFormControl(defaults.voucherType, Validators.required));
        this.form.addControl('useAmountFromFile', useAmountFromFileControl);
        this.form.addControl('amount', new UntypedFormControl(defaults.amount, AMOUNT_VALIDATOR));
        this.form.addControl('sendMessage', new UntypedFormControl(defaults.sendMessage));
        break;
    }
