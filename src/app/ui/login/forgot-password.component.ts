...
interface ForgotPasswordRequestFormControls {
  user: FormControl<string | null>;
  sendMedium: FormControl<SendMediumEnum | null>;
  captcha?: AbstractControl<any, any>;
}

interface ForgotPasswordCodeFormControls {
  user: FormControl<string | null>;
  code: FormControl<string | null>;
}

interface ForgotPasswordChangeFormControls {
  user: FormControl<string | null>;
  code: FormControl<string | null>;
  securityAnswer?: FormControl<string | null>;
  checkConfirmation?: FormControl<boolean | null>;
  newPassword?: FormControl<string | null>;
  newPasswordConfirmation?: FormControl<string | null>;
  sendMedium?: FormControl<SendMediumEnum | null>;
}

type ForgotPasswordRequestForm = FormGroup<ForgotPasswordRequestFormControls>;
type ForgotPasswordCodeForm = FormGroup<ForgotPasswordCodeFormControls>;
type ForgotPasswordChangeForm = FormGroup<ForgotPasswordChangeFormControls>;
type ForgotPasswordForm = ForgotPasswordRequestForm | ForgotPasswordCodeForm | ForgotPasswordChangeForm;
...
export class ForgotPasswordComponent extends BasePageComponent<DataForLogin> implements OnInit {
  ...
  requestForm: ForgotPasswordRequestForm;
  codeForm: ForgotPasswordCodeForm;
  sendMedium: SendMediumEnum;
  changeData: DataForChangeForgottenPassword;
  changeForm: ForgotPasswordChangeForm;
  ...
  onDataInitialized(data: DataForLogin) {
    this.requestForm = new FormGroup<ForgotPasswordRequestFormControls>({
      user: new FormControl<string | null>(null, Validators.required),
      sendMedium: new FormControl<SendMediumEnum | null>(data.forgotPasswordMediums[0] ?? null)
    });
  ...
  toCode() {
    ...
    const params = this.requestForm.getRawValue() as ForgottenPasswordRequest;
    ...
    this.errorHandler.requestWithCustomErrorHandler(() =>
      this.authService.forgottenPasswordRequest({ body: params }).subscribe(
        resp => {
          ...
          this.codeForm = new FormGroup<ForgotPasswordCodeFormControls>({
            user: new FormControl<string | null>(this.requestForm.controls.user.value, Validators.required),
  ...
  toChange() {
    ...
    const params = this.codeForm.getRawValue();
    params.user = ApiHelper.escapeNumeric(params.user);
    this.errorHandler.requestWithCustomErrorHandler(defaultHandling => {
      this.authService.getDataForChangeForgottenPassword(params).subscribe(
        changeData => {
          ...
          this.changeForm = new FormGroup<ForgotPasswordChangeFormControls>({
            user: new FormControl<string | null>(this.codeForm.controls.user.value, Validators.required),
            code: new FormControl<string | null>(this.codeForm.controls.code.value, Validators.required)
          });
          if (changeData.securityQuestion) {
            this.changeForm.addControl('securityAnswer', new FormControl<string | null>(null, Validators.required));
          }
          if (changeData.passwordType.mode === PasswordModeEnum.MANUAL) {
            this.changeForm.addControl('checkConfirmation', new FormControl<boolean | null>(true));
            this.changeForm.addControl('newPassword', new FormControl<string | null>(null, Validators.required));
            this.changeForm.addControl('newPasswordConfirmation', new FormControl<string | null>(null, Validators.required));
          } else {
            this.changeForm.addControl(
              'sendMedium',
              new FormControl<SendMediumEnum | null>(this.requestForm.controls.sendMedium.value)
            );
          }
          this.step = 'change';
        },
  ...
  change() {
    ...
    const params = this.changeForm.getRawValue() as ChangeForgottenPassword;
    params.user = ApiHelper.escapeNumeric(params.user);
    this.addSub(
