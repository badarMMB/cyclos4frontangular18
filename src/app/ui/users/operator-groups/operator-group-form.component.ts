  ...
  implements OnInit
{
  ...
  tokenPermissions: UntypedFormControl;
  ...
  onDataInitialized(data: OperatorGroupDataForEdit | OperatorGroupDataForNew) {
    ...
    if (!empty(data.accountTypes)) {
      ...
      const accountControls: UntypedFormGroup = this.formBuilder.group({});
    ...
    if (!empty(data.paymentTypes)) {
      const paymentControls: UntypedFormGroup = this.formBuilder.group({});
    ...
    this.form.addControl('editOwnProfile', new UntypedFormControl(group.editOwnProfile));
    this.form.addControl('restrictPaymentsToUsers', new UntypedFormControl(group.restrictPaymentsToUsers));
    if (data.canHaveNotifications) {
      this.form.addControl('notifications', new UntypedFormControl(group.notifications));
    }
    if (data.canChargebackPayments) {
      this.form.addControl('chargebackPayments', new UntypedFormControl(group.chargebackPayments));
    }
    if (data.canReceivePayments) {
      this.form.addControl('receivePayments', new UntypedFormControl(group.receivePayments));
    }
    if (data.canRequestPayments) {
      this.form.addControl('requestPayments', new UntypedFormControl(group.requestPayments));
    }
    if (data.canViewAdvertisements) {
      this.form.addControl('viewAdvertisements', new UntypedFormControl(group.viewAdvertisements));
    }
    if (data.canManageAdvertisements) {
      this.form.addControl('manageAdvertisements', new UntypedFormControl(group.manageAdvertisements));
    }
    if (data.canHaveMessages) {
      this.form.addControl('messages', new UntypedFormControl(group.messages));
    }
    ...
    this.tokenPermissions = new UntypedFormControl(tokenOptions);

    if (data.canPerformVoucherTransactions) {
      this.form.addControl('voucherTransactions', new UntypedFormControl(group.voucherTransactions));
    }
    if (data.broker) {
      this.form.addControl('brokering', new UntypedFormControl(group.brokering));
    }
    if (!empty(data.operations)) {
      this.form.addControl('operations', new UntypedFormControl(group.operations));
    }
    if (!empty(data.recordTypes)) {
      this.form.addControl('records', new UntypedFormControl(group.records));
    }
    if (group['version']) {
      this.form.addControl('version', new UntypedFormControl(group['version']));
    }
  }
