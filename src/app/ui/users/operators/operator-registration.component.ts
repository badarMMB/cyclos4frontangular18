...
export class OperatorRegistrationComponent extends BasePageComponent<OperatorDataForNew> implements OnInit {
  ...
  register() {
    ...
    if (this.mobileForm) {
      fullForm.setControl('mobilePhones', new UntypedFormArray([this.mobileForm]));
    }
    if (this.landLineForm) {
      fullForm.setControl('landLinePhones', new UntypedFormArray([this.landLineForm]));
    }
    fullForm.setControl('passwords', new UntypedFormArray(this.passwordForms));
