...
export class EditProfileComponent extends BasePageComponent<DataForEditFullProfile> implements OnInit {
  ...
  createLandLinePhones: UntypedFormGroup[] = [];
  modifyLandLinePhones: UntypedFormGroup[] = [];
  createMobilePhones: UntypedFormGroup[] = [];
  modifyMobilePhones: UntypedFormGroup[] = [];
  createAddresses: UntypedFormGroup[] = [];
  modifyAddresses: UntypedFormGroup[] = [];
  createContactInfos: UntypedFormGroup[] = [];
  modifyContactInfos: UntypedFormGroup[] = [];

  // Additional form controls
  confirmationPassword: UntypedFormControl;
  defineSingleAddress: UntypedFormControl;
  singleMobile: UntypedFormGroup;
  singleLandLine: UntypedFormGroup;
  singleAddress: UntypedFormGroup;
  ...
  resolveFormGroup(): UntypedFormGroup {
    return new UntypedFormGroup({
      user: this.user,
      createLandLinePhones: new UntypedFormArray(this.createLandLinePhones),
      modifyLandLinePhones: new UntypedFormArray(this.modifyLandLinePhones),
      createMobilePhones: new UntypedFormArray(this.createMobilePhones),
      modifyMobilePhones: new UntypedFormArray(this.modifyMobilePhones),
      createAddresses: new UntypedFormArray(this.createAddresses),
      modifyAddresses: new UntypedFormArray(this.modifyAddresses),
      createContactInfos: new UntypedFormArray(this.createContactInfos),
      modifyContactInfos: new UntypedFormArray(this.modifyContactInfos)
    });
  }
  ...
  private buildUserForm(): UntypedFormGroup {
  ...
  private stampDataAndInitForm(model: any, form: UntypedFormGroup) {
    model.idSuffix = `_${model.id || modelIndex++}`;
    model.form = form;
    form.patchValue(model);
  }

  private buildLandLineForm(phone: PhoneManage): UntypedFormGroup {
  ...
  private buildMobileForm(phone: PhoneManage): UntypedFormGroup {
  ...
  private buildPhoneForm(single: boolean, required: boolean): UntypedFormGroup {
  ...
  private buildAddressForm(address: AddressManage): UntypedFormGroup {
    const data = this.data.addressConfiguration;
  ...
  private buildContactInfoForm(contactInfo: ContactInfoManage): UntypedFormGroup {
    const data = this.data.contactInfoConfiguration;
    const form = this.formBuilder.group({
      id: null,
      version: null,
  ...
  public showEnableForSms(phoneForm: UntypedFormGroup) {
    if (!this.data.phoneConfiguration.smsEnabled) {
      return false;
    }
    if (this.byManager) {
