...
export class UserRegistrationComponent extends BasePageComponent<UserDataForNew> implements OnInit, OnDestroy {
  ...
  landLineForm: UntypedFormGroup;
  defineAddress: UntypedFormControl;
  ...
  private initializeGroups(groups: (GroupForRegistration | Group)[]) {
    ...
    if (empty(this.groups)) {
      ...
      this.group = new UntypedFormControl(groups[0].id, Validators.required);
      if (groups.length === 1) {
  ...
  private prepareForms(data: UserDataForNew) {
    ...
    if (imageAvailability !== AvailabilityEnum.DISABLED) {
      this.imageControl = new UntypedFormControl(
        null,
  ...
  showConfirm() {
    ...
    const fullForm = new UntypedFormGroup({
      user: this.form
    });
    ...
    const nonValid = validateBeforeSubmit(fullForm, true) as UntypedFormControl[];
    this.addSub(
  ...
  register() {
    const nonValid = validateBeforeSubmit(this.confirmForm, true) as UntypedFormControl[];
    this.addSub(
      mergeValidity(nonValid).subscribe(isValid => {
        if (isValid) {
  ...
  get userNew(): UserNew {
    ...
    const mobile: PhoneNew = (this.mobileForm || ({} as UntypedFormGroup)).value;
    const landLine: PhoneNew = (this.landLineForm || ({} as UntypedFormGroup)).value;
    const address: AddressNew = (this.addressForm || ({} as UntypedFormGroup)).value;
    if (this.image) {
