...
export class RunWizardStepFieldComponent extends BaseComponent implements OnInit {
  ...
  private _user: UntypedFormGroup;
  @Input() get user(): UntypedFormGroup {
    return this._user;
  }
  set user(user: UntypedFormGroup) {
    this._user = user;
    this.customProfileValues = user ? (user.controls.customValues as UntypedFormGroup) : null;
  }
  @Input() mobilePhone: UntypedFormGroup;
  @Input() landLinePhone: UntypedFormGroup;
  @Input() defineAddress: UntypedFormControl;
  @Input() address: UntypedFormGroup;
  @Input() defineContactInfo: UntypedFormControl;
  @Input() contactInfo: UntypedFormGroup;
  @Input() customValues: UntypedFormGroup;
  @Input() emailValidation: UntypedFormControl;
  @Input() smsValidation: UntypedFormControl;
  ...
  customProfileValues: UntypedFormGroup;
  ...
  ngOnInit() {
    ...
    if (this.data.wizard.kind === WizardKind.REGISTRATION) {
      ...
      if (this.field.customProfileField) {
        ...
        this.label = this.customField.name;
        const customValues = this.user.controls.customValues as UntypedFormGroup;
  ...
  passwordForm(index: number): UntypedFormGroup {
    const passwords = this.user ? (this.user.controls.passwords as UntypedFormArray) : null;
    return passwords ? (passwords.controls[index] as UntypedFormGroup) : null;
  }
  ...
  private doSendCode(button: CountdownButtonComponent, control: AbstractControl, medium: SendMediumEnum) {
    const nonValid = validateBeforeSubmit(control, true) as UntypedFormControl[];
